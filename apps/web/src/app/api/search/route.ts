...
export async function GET(request: NextRequest) {
  ...
  if (!keyword.trim()) {
    return NextResponse.json({
      ok: false, keyword: '', dryRun: false,
      sourceCount: 0, successSourceCount: 0, failedSourceCount: 0,
      durationMs: 0, results: [], errors: [
        { sourceId: '*', sourceName: 'input', reason: 'keyword required', scope: 'search-api' },
      ],
    });
  }
  ...
  try {
    ...
    return NextResponse.json({
      ...
      sourceCount: 0, successSourceCount: 0, failedSourceCount: 0,
      durationMs: 0, results: [], errors: [
        { sourceId: '*', sourceName: 'server', reason: `fatal: ${error instanceof Error ? error.message : String(error)}`, scope: 'search-api' },
      ],
    });
  }
}
